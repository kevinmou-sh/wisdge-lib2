...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
	...
	public static String GBK2BIG(String str) {
		...
		try {
			return new String(str.getBytes("GBK"), "BIG5");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
			return "";
		}
	...
	public static List<String> toList(String source, String splitRegex) {
		List<String> set = new ArrayList<>();
		if (StringUtils.isEmpty(source))
			return set;

		for(String sub : source.split(splitRegex)) {
			set.add(sub.trim());
		}
		return set;
	}
